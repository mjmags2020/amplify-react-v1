...
function Contacts() {
    ...
    const getContacts = async () => {
        try {
            const contactsData = await client.graphql(graphqlOperation(listContacts));
            ...
            contacts.map(async (contact, indx) => {
                ...
                try {
                    const contactProfilePicPathURI = await getUrl(contactProfilePicPath, { expires: 60 });
    ...
    const addNewContact = async () => {
        try {
            ...
            // Persist new Contact
            await client.graphql(graphqlOperation(createContact, { input: newContact }));
        } catch (err) {
            console.log('error', err);
        }
    }
